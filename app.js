...
function classesRaw() { return DB.get("sms_classes", []); }
function saveClassesRaw(c) { DB.set("sms_classes", c); }
function classesFromUsers() {
  const fromStudents = students().map(s => s.className).filter(Boolean);
  return [...new Set([...fromStudents, ...classesRaw()])].sort((a, b) => a.localeCompare(b, "ru"));
}
function classStudentCount(c) { return students().filter(s => s.className === c).length; }
...
function renderStudents() {
  ...
  return `
    ...
    <div class="card" style="margin-bottom:18px;">
      <div class="card-head">
        <h3>Классы <span class="card-sub" style="font-weight:400;">(${classes.length})</span></h3>
        <button class="btn btn-ghost btn-sm" id="addClassBtn">+ Новый класс</button>
      </div>
      <div class="chips-row">
        ${classes.length ? classes.map(c => {
          const cnt = classStudentCount(c);
          return `<span class="class-chip">${esc(c)} <span class="cnt">${cnt}</span>${cnt === 0 ? `<button class="chip-del" data-del-class="${esc(c)}" title="Удалить пустой класс">×</button>` : ""}</span>`;
        }).join("") : `<span class="text-muted" style="font-size:13px;">Классов пока нет — создайте первый, не дожидаясь учеников.</span>`}
      </div>
    </div>
    <div class="card">
      <div class="card-head">
...
function classFormModal() {
  openModal(`
    <h3>Новый класс</h3>
    <p class="modal-sub">Создайте класс заранее — учеников и расписание можно добавить позже.</p>
    <form id="classForm">
      <div class="form-row"><label>Название класса</label><input type="text" id="cfName" required placeholder="например, 9-A" autofocus></div>
      <p id="cfError" class="login-error"></p>
      <div class="modal-actions">
        <button type="button" class="btn btn-ghost" id="cfCancel">Отмена</button>
  ...
  document.getElementById("classForm").onsubmit = e => {
    ...
    const existing = classesFromUsers();
    if (existing.some(c => c.toLowerCase() === name.toLowerCase())) {
      document.getElementById("cfError").textContent = "Такой класс уже существует.";
      return;
    }
    const list = classesRaw();
    list.push(name);
    saveClassesRaw(list);
...
function bindSectionEvents(user) {
  // Students
  document.getElementById("addClassBtn")?.addEventListener("click", () => classFormModal());
  document.querySelectorAll("[data-del-class]").forEach(b => b.addEventListener("click", () => {
    const name = b.dataset.delClass;
    if (classStudentCount(name) > 0) { toast("Нельзя удалить класс, в котором есть ученики"); return; }
    saveClassesRaw(classesRaw().filter(c => c !== name));
    toast("Класс удалён");
    render();
  }));
